...
if __name__=="__main__":
    ...
    if len(sys.argv) < 4:
        ...
        with rosbag.Bag('output2.bag', 'w') as outbag:
            i=0
            msg_ref_pub = Path()
            for topic_ref, msg_ref, t in rosbag.Bag(sys.argv[1]).read_messages(topic_ref):
                msg_ref_pub.header = msg_ref.header
                pose_ref_pub = PoseStamped()
                pose_ref_pub.header = msg_ref.header
                pose_ref_pub.pose.position.x=traj_ref.positions_xyz[i,0]
                pose_ref_pub.pose.position.y=traj_ref.positions_xyz[i,1]
                pose_ref_pub.pose.position.z=traj_ref.positions_xyz[i,2]
                pose_ref_pub.pose.orientation.w=traj_ref.orientations_quat_wxyz[i,0]
                pose_ref_pub.pose.orientation.x=traj_ref.orientations_quat_wxyz[i,1]
                pose_ref_pub.pose.orientation.y=traj_ref.orientations_quat_wxyz[i,2]
                pose_ref_pub.pose.orientation.z=traj_ref.orientations_quat_wxyz[i,3]
                msg_ref_pub.poses.append(pose_ref_pub)
                i=i+1
            ...
            for topic_est, msg_est, t in rosbag.Bag(sys.argv[1]).read_messages(topic_est):
                msg_est_pub.header = msg_est.header
                pose_est_pub = PoseStamped() 
                pose_est_pub.header = msg_est.header
                pose_est_pub.pose.position.x=traj_est.positions_xyz[i,0]
                pose_est_pub.pose.position.y=traj_est.positions_xyz[i,1]
                pose_est_pub.pose.position.z=traj_est.positions_xyz[i,2]
                pose_est_pub.pose.orientation.w=traj_est.orientations_quat_wxyz[i,0]
                pose_est_pub.pose.orientation.x=traj_est.orientations_quat_wxyz[i,1]
                pose_est_pub.pose.orientation.y=traj_est.orientations_quat_wxyz[i,2]
                pose_est_pub.pose.orientation.z=traj_est.orientations_quat_wxyz[i,3]
                msg_est_pub.poses.append(pose_est_pub)
                i=i+1
